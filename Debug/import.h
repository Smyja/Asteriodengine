...
if (objsm[q - 1] != '_'&&objsm[q] == 'K'&&objsm[q + 1] == 's'&&w<1000000) {
	...
	gauv[b] = (vectz*)malloc(sizeof(vectz*) * (nin * 3));
	scal[b] = (vectz*)malloc(sizeof(vectz*) * (nin * 3));
	rota[b] = (vectz*)malloc(sizeof(vectz*) * (nin * 3));
...
free5=bued;	bued+=bu; obcc[b]=bu*1.0/8.0;
	...
	ou = 0; bu = 0;
	handx[0] = 0; handy[0] = 0; handz[0]=0;

	for (int a = 0,q=0,r=0;a < m;a++){
		if (fnice[a]<0) { q = 0; }
		if (fnice[a]==-3&&r>0){
	handx[bu]/=r; handy[bu] /= r; handz[bu] /= r;
	handx[bu+1]=0; handy[bu+1] = 0; handz[bu + 1] = 0;
	bu++; r=0; }
		if(q==1){
			handx[bu]+=bufary[(ou*8)+free5];
			handy[bu]+=bufary[(ou * 8)+1+free5];
			handz[bu]+=bufary[(ou * 8)+2+free5];
			ou++; r++;
		}
		if(fnice[a]==-1){ q = 1; }
		if(a>=m-1){ handx[bu]/=r; handy[bu]/=r; handz[bu]/=r; }
	}
	sprintf(how3,"");
	for(int a=0;a<m;a++){ sprintf(how3,"%s %d",how3,fnice[a]); }
	MessageBox(hwnd,how3,NULL,NULL);
	center[b] = (vectz*)malloc(sizeof(vectz*) * ((bu+1) * 3));
	for(int a=0;a<=bu;a++){ 
	center[b][a].x=handx[a];  
	center[b][a].y = handy[a]; 
	center[b][a].z = handz[a]; 
}
	gcont[b]=bu;
